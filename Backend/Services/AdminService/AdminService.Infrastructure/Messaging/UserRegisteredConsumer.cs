            ...
            consumer.ReceivedAsync += async (_, ea) =>
            {
                try
                {
                    ...
                    if (evt is not null)
                    {
                        ...
                        // If role is DeliveryAgent, also create DeliveryAgentSnapshot
                        if (evt.Role.Equals("DeliveryAgent", StringComparison.OrdinalIgnoreCase))
                        {
                            var agentRepo = s.ServiceProvider
                                              .GetRequiredService<IDeliveryAgentSnapshotRepository>();
                            ...
                            _log.LogInformation(
                                "[Admin Consumer] DeliveryAgentSnapshot created for pending approval: {Id}.",
                                evt.UserId);
                        }
                    }
...
public class OrderPlacedConsumer : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken ct)
    {
        try
        {
            ...
            await channel.QueueDeclareAsync(QueueNames.OrderPlacedAdmin,
                durable: true, exclusive: false, autoDelete: false, cancellationToken: ct);
            await channel.BasicQosAsync(0, 1, false, ct);
            ...
            consumer.ReceivedAsync += async (_, ea) =>
            {
                try
                {
                    ...
                    if (evt is not null)
                    {
                        ...
                        // Try to look up restaurant name from RestaurantSnapshot
                        var restaurantRepo = s.ServiceProvider
                                               .GetRequiredService<IRestaurantSnapshotRepository>();
                        var restaurant = await restaurantRepo.GetByIdAsync(evt.RestaurantId);
                        ...
                        await orderRepo.UpsertAsync(new OrderSnapshot
                        {
                            ...
                            CustomerName = customer?.FullName ?? string.Empty,  // Added customer name
                            RestaurantId = evt.RestaurantId,
                            RestaurantName = restaurant?.Name ?? string.Empty,  // Get restaurant name from snapshot
            ...
            await channel.BasicConsumeAsync(QueueNames.OrderPlacedAdmin, autoAck: false, consumer: consumer, cancellationToken: ct);
...
public class AdminPaymentCompletedConsumer : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken ct)
    {
        try
        {
            ...
            // Listen to the admin-specific payment.completed queue
            await channel.QueueDeclareAsync(QueueNames.PaymentCompletedAdmin, durable: true, exclusive: false, autoDelete: false, cancellationToken: ct);
            await channel.BasicQosAsync(0, 1, false, ct);
            ...
            consumer.ReceivedAsync += async (_, ea) =>
            {
                try
                {
                    ...
                    if (evt is not null)
                    {
                        ...
                        await orderRepo.SaveChangesAsync();
                        _log.LogInformation("[Admin Consumer] OrderSnapshot {OrderId} updated to Paid", evt.OrderId);
                    }
            ...
            await channel.BasicConsumeAsync(QueueNames.PaymentCompletedAdmin, autoAck: false, consumer: consumer, cancellationToken: ct);
            _log.LogInformation("[Admin Consumer] AdminPaymentCompletedConsumer started listening to {Queue}.", QueueNames.PaymentCompletedAdmin);
...
public class AdminDeliveryStatusConsumer : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken ct)
    {
        try
        {
            ...
            const string queue = "delivery.status_updated";
